...
public class CreateUser extends ConfigClass{
	...
	public void createUser(String emp_name,String uname,String pword,String cpword) throws InterruptedException {
		...
		//Select user role
		Select select = new Select(driver.findElement(By.id(Locators.user_role)));
		select.selectByVisibleText(InstanceVariables.user_role);

		//Input employee name
		WebElement employee_name = driver.findElement(By.id(Locators.employee_name));
		employee_name.clear();
		employee_name.sendKeys(emp_name);

		//Input username
		WebElement username = driver.findElement(By.id(Locators.username));
		username.clear();
		username.sendKeys(uname);

		//Input password
		WebElement password = driver.findElement(By.id(Locators.password));
		password.clear();
		password.sendKeys(pword);

		//Input confirm password
		WebElement confirm_password = driver.findElement(By.id(Locators.confirm_password));
		confirm_password.clear();
		confirm_password.sendKeys(cpword);

		//Save button
		WebElement save_button = driver.findElement(By.name(Locators.save_button));
		Thread.sleep(3000);
		save_button.click();
	...
	public void validateStatusDropDown(){
		...
		//Creates a test
		test = extent.createTest("Status dropdown validation","Verifying if status dropdown is selected to " + InstanceVariables.desired_selected_status);

		//Locates the select on the webpage
		Select status_select = new Select(driver.findElement(By.id(Locators.status_dropdown)));
		...
		if(status_select.getFirstSelectedOption().getText().equalsIgnoreCase(InstanceVariables.desired_selected_status)) {
			result = true;
			message = "Status dropdown is selected to " + InstanceVariables.desired_selected_status;
		}
		else {
			result = false;
			message = "Status dropdown is not selected to " + InstanceVariables.desired_selected_status;
		}
	...
	public void validateUserCreated() throws InterruptedException {
		...
		//Creates a test
		test = extent.createTest("Create user validation","Validating if the user was created by searching through the record");
		...
		WebElement record_uname = driver.findElement(By.xpath(Locators.record_username));
